...
public class OrdersService {
	...
	public List<OrdersDTO> getOrders() {
		...
		return orders.stream()
				.map(order -> new OrdersDTO(order.getOrderId(), order.getMember().getUsername(),
						...
						order.getRequestDate(), order.getMemo(), order.getSelectedDay(), null // OrderDetail은 포함되지 않음
	...
	public List<OrdersDTO> getOrdersWithDetails() {
	    ...
	    return orders.stream().map(order -> {
	        ...
	        List<OrderDetailDTO> orderDetailDTOs = convertOrderDetailToDTO(orderDetails, order.getReleaseDate(), order.getSelectedDay());

	        return new OrdersDTO(order.getOrderId(), order.getMember().getUsername(), order.getMember().getAlias(),
	                order.getMember().getPhone(), order.getReleaseDate(), order.getBestOrderDate(), order.getRequestDate(), order.getMemo(), order.getSelectedDay(),
	                orderDetailDTOs);
	    }).collect(Collectors.toList());
	}
	...
	public OrdersDTO getOrderDetailsByOrderId(Integer orderId, Authentication authentication) {
	 ...
	 // bestOrderDate 계산
	    LocalDate bestOrderDate = calculateBestOrderDate(filteredOrderDetails, order.getReleaseDate(), order.getSelectedDay());
	    ...
	    List<OrderDetailDTO> orderDetailDTOs = filteredOrderDetails.stream().map(orderDetail -> {
	        ...
	        System.out.println("리드타임 조회: itemsId = " + item.getItemsId() +
	                   ", selectedDay = " + order.getSelectedDay() +
	                   ", releaseDate = " + order.getReleaseDate());

	        List<Leadtime> leadtimes = null;
	        try {
	            // 리드타임 조회
	            leadtimes = leadtimeRepo.findLeadtimeByItemsIdAndSelectedDayAndSeason(
	                item.getItemsId(),
	    ...
	    return new OrdersDTO(
	        ...
	        order.getMemo(),
	        order.getSelectedDay(),
	        orderDetailDTOs // 리드타임이 포함된 DTO 리스트
	    );
	}
	...
	// bestOrderDate 계산 로직
	private LocalDate calculateBestOrderDate(List<OrderDetail> orderDetails, LocalDate releaseDate, SelectedDay selectedDay) {
	    ...
	    return orderDetails.stream().map(orderDetail -> {
	        ...
	        // 리드타임 조회
	        List<Leadtime> leadtimes = leadtimeRepo.findLeadtimeByItemsIdAndSelectedDayAndSeason(
	            item.getItemsId(),
	            selectedDay,
	            releaseDate
	        );
	...
	// OrderDetail을 DTO로 변환하는 로직 (리드타임을 조건에 맞춰 설정)
	private List<OrderDetailDTO> convertOrderDetailToDTO(List<OrderDetail> orderDetails, LocalDate releaseDate, SelectedDay selectedDay) {
	    return orderDetails.stream().map(orderDetail -> {
	        var item = orderDetail.getItem();
